...
Reads molecule files in the directory `joinpath(PATH_TO_DATA, PATH_TO_MOLECULES, species)`.
...
function Molecule(species::AbstractString; assert_charge_neutrality::Bool=true)
    if ! isdir(joinpath(PATH_TO_DATA, PATH_TO_MOLECULES, species))
        error(@sprintf("No directory created for %s in %s\n", species,
                       joinpath(PATH_TO_DATA, PATH_TO_MOLECULES)))
    end

    ###
    #  Read in Lennard Jones spheres
    ###
    atomsfilename = joinpath(PATH_TO_DATA, PATH_TO_MOLECULES, species, "lennard_jones_spheres.csv")
    ...
    ###
    chargesfilename = joinpath(PATH_TO_DATA, PATH_TO_MOLECULES, species, "point_charges.csv")
