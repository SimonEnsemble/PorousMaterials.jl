...
function vdw_energy(
    ...
    return vdw_energy(Frac(atoms_i, box), Frac(atoms_j, box), box, ljff)
end
function vdw_energy(
    atoms_i::Atoms{Frac},
    atoms_j::Atoms{Cart},
    ...
    return vdw_energy(atoms_i, Frac(atoms_j, box), box, ljff)
end
function vdw_energy(
    atoms_i::Atoms{Cart},
    atoms_j::Atoms{Frac},
    ...
    return vdw_energy(Frac(atoms_i, box), atoms_j, box, ljff)
end
...
function vdw_energy(
    ...
    for other_molecule_id in eachindex(molecules)
        ...
        energy += vdw_energy(
            ...
            molecules[other_molecule_id].atoms,
            box,
            ljff
        )
    end
...
function vdw_energy(
    ...
    for s in 1:length(molecules)
        ...
        for m in 1:length(molecules[s])
            ...
            energy += vdw_energy(
                molecules[species_id][molecule_id].atoms,
                molecules[s][m].atoms,
                box,
                ljff
            )
