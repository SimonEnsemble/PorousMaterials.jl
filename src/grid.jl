...
function id_to_xf(id::Tuple{Int, Int, Int}, n_pts::Tuple{Int, Int, Int})
    return [(id[k] - 1.0) / (n_pts[k] - 1.0) for k in 1:3]
end
...
"""
function update_density!(grid::Grid, molecules::Array{Molecule{Frac}, 1}, species::Symbol)
    for molecule in molecules
        for i in 1:(molecule.atoms.n)
            if molecule.atoms.species[i] == species
                voxel_id = xf_to_id(grid.n_pts, molecule.atoms.coords.xf[:, i])
...
function required_n_pts(box::Box, dx::Float64)
    ...
    cell_vector_norms = [norm(box.f_to_c[:, i]) for i in 1:3]
    n_pts = zeros(Int, 3)
    for i in 1:3
...
function write_cube(
    ...
    for i in 1:grid.n_pts[1]
        for j in 1:grid.n_pts[2]
            for k in 1:grid.n_pts[3]
...
function read_cube(filename::AbstractString; has_units::Bool=true)
    ...
    origin = [parse(Float64, split(line)[1 + i]) for i in 1:3]

    # read box info
    box_lines = [readline(cubefile) for _ in 1:3]

    # number of grid pts
    n_pts = Tuple([parse(Int, split(box_lines[i])[1]) for i in 1:3])
...
function energy_grid(
    ...
    n_pts = if isa(resolution, Tuple{Int, Int, Int})
        resolution
    else
        required_n_pts(crystal.box, resolution)
    end
    ...
    grid_pts = [collect(range(0.0; stop=1.0, length=n_pts[i])) for i in 1:3]
    ...
    if verbose
        ...
        @printf(
            "\tRegular grid (in fractional space) of %d by %d by %d points superimposed over the unit cell.\n",
            n_pts[1],
            n_pts[2],
            n_pts[3]
        )
        if rotations_required
            @printf(
                "\t%d molecule rotations per grid point with temperature %f K.\n",
                n_rotations,
                temperature
            )
        end
...
function Base.show(io::IO, grid::Grid)
    @printf(
        io,
        "Regular grid of %d by %d by %d points superimposed over a unit cell and associated data.\n",
        grid.n_pts[1],
        grid.n_pts[2],
        grid.n_pts[3]
    )
...
function Base.isapprox(
    ...
    g2::Grid;
    atol::Real=0.0,
    rtol::Real=atol > 0.0 ? 0.0 : sqrt(eps())
)
    return (
        isapprox(g1.box, g2.box; atol=atol, rtol=rtol) &&
        (g1.n_pts == g2.n_pts) &&
...
function _flood_fill!(
    ...
    for i_s in -1:1, j_s in -1:1, k_s in -1:1
...
function _segment_grid(grid::Grid, energy_tol::Float64, verbose::Bool)
    ...
    segmented_grid =
        Grid(grid.box, grid.n_pts, zeros(Int, grid.n_pts...), :Segment_No, grid.origin)
    segment_no = 0
    for i in 1:grid.n_pts[1], j in 1:grid.n_pts[2], k in 1:grid.n_pts[3]
        if segmented_grid.data[i, j, k] == 0 # not yet assigned segment
            if grid.data[i, j, k] > energy_tol # not accessible
                ...
                while length(queue_of_grid_pts) != 0
                    ...
                    _flood_fill!(grid, segmented_grid, queue_of_grid_pts, id..., energy_tol)
...
function _note_connection!(
    segment_1::Int,
    segment_2::Int,
    connections::Array{SegmentConnection, 1},
    ...
    if !(segment_connection in connections)
        ...
        if verbose
            @printf(
                "Noted seg. %d --> %d connection in (%d, %d, %d) direction.\n",
                segment_1,
                segment_2,
                direction[1],
                direction[2],
        ...
        push!(
            connections,
            SegmentConnection(segment_2, segment_1, segment_connection.direction .* -1)
        )
    end
...
function _build_list_of_connections(segmented_grid::Grid; verbose::Bool=true)
    ...
    for i in 1:segmented_grid.n_pts[1], j in 1:segmented_grid.n_pts[2]
        _note_connection!(
            segmented_grid.data[i, j, end],
            segmented_grid.data[i, j, 1],
            connections,
            (0, 0, 1),
            verbose
        )
    ...
    for j in 1:segmented_grid.n_pts[2], k in 1:segmented_grid.n_pts[3]
        _note_connection!(
            segmented_grid.data[end, j, k],
            segmented_grid.data[1, j, k],
            connections,
            (1, 0, 0),
            verbose
        )
    ...
    for i in 1:segmented_grid.n_pts[1], k in 1:segmented_grid.n_pts[3]
        _note_connection!(
            segmented_grid.data[i, end, k],
            segmented_grid.data[i, 1, k],
            connections,
            (0, 1, 0),
            verbose
        )
...
function _translate_into_graph(
    segmented_grid::Grid,
    connections::Array{SegmentConnection, 1}
)
...
function _classify_segments(
    ...
    if verbose
        @printf(
            "\tFound %d simple cycles in segment connectivity graph.\n",
            length(all_cycles)
        )
    end
...
function _assign_inaccessible_pockets_minus_one!(
    segmented_grid::Grid,
    segment_classifiction::Array{Int, 1};
    verbose::Bool=true
)
    for s in eachindex(segment_classifiction)
        if segment_classifiction[s] == 1
            if verbose
...
"""
function compute_accessibility_grid(
    crystal::Crystal,
    probe::Molecule,
    forcefield::LJForceField;
    resolution::Union{Float64, Tuple{Int, Int, Int}}=1.0,
    energy_tol::Float64=10.0,
    energy_units::Symbol=:kJ_mol,
    verbose::Bool=true,
    ...
    if !block_inaccessible_pockets
        accessibility_grid = Grid{Bool}(
            grid.box,
            grid.n_pts,
            grid.data .< energy_tol,
            :accessibility,
            grid.origin
        )
    ...
    if write_b4_after_grids
        ...
        gridfilename = @sprintf(
            "%s_in_%s_%s_b4_pocket_blocking.cube",
            probe.species,
    ...
    segment_classifications =
        _classify_segments(segmented_grid, graph, vertex_to_direction, verbose)

    # assign inaccessible pockets minus one if cycle not found in graph
    _assign_inaccessible_pockets_minus_one!(
        segmented_grid,
        segment_classifications;
        verbose=verbose
    )
    ...
    if write_b4_after_grids
        gridfilename = @sprintf(
            "%s_in_%s_%s_after_pocket_blocking.cube",
            probe.species,
            replace(replace(crystal.name, ".cif" => ""), ".cssr" => ""),
            replace(forcefield.name, ".csv" => "")
        )
    ...
    if all(segmented_grid.data .== -1)
        @warn @sprintf(
            "%s cannot enter the pores of %s with %f K energy tolerance.",
            probe.species,
            crystal.name,
            energy_tol
        )
    ...
    accessibility_grid = Grid{Bool}(
        segmented_grid.box,
        segmented_grid.n_pts,
        segmented_grid.data .== 1,
        :accessibility,
        segmented_grid.origin
    )

    # compute porosity before and after blocking
    porosity = Dict(
        :b4_blocking => sum(grid.data .< energy_tol) / length(grid.data),
        :after_blocking =>
    ...
    if nb_segments_blocked != 0
        ...
        @printf(
            "\tPorosity of %s b4 pocket blocking is %f\n",
            crystal.name,
            porosity[:b4_blocking]
        )
        @printf(
            "\tPorosity of %s after pocket blocking is %f\n",
            crystal.name,
            porosity[:after_blocking]
        )
        @printf(
            "\tpotential energy barrier used to determine accessibility: %f %s\n",
            energy_tol,
            energy_units
        )
    end
