...
function GCMCstats(nb_species::Int)
    return GCMCstats(
        0,
        zeros(Int, nb_species),
        zeros(Int, nb_species),
...
function +(s1::GCMCstats, s2::GCMCstats)
    return GCMCstats(
...
function Base.print(gcmc_stats::GCMCstats)
    ...
    return println("\t⟨U⟩ (K) = ", sum(gcmc_stats.U) / gcmc_stats.n_samples)
end
...
function μVT_sim(
    ...
    ###
    num_snapshots = 0
    xyz_snapshots_filename = μVT_output_filename(
        xtal,
        molecule_templates,
        temperature,
        pressures,
    ...
    fugacities = [NaN for s in 1:nb_species] # Pa
    if eos == :ideal
    ...
    elseif eos == :PengRobinson
        ...
        error(
            "eos=:ideal and eos=:PengRobinson are the only valid options for an equation of state."
        )
    end
    if verbose
        for s in 1:nb_species
            @printf(
                "\t%s equation of state, %s partial fugacity = %f bar\n",
                eos,
                molecule_templates[s].species,
                fugacities[s] / 100000.0
            )
        end
    end
    ...
    if isnothing(molecules)
        ...
        if length(molecules) != nb_species
            error(
                "Length of molecules array $(length(molecules)) is not equal to length of molecule_templates $(nb_species).\n"
            )
        end
    end
    ...
    if calculate_density_grid && isnothing(density_grid_molecular_species)
        if nb_species == 1
            ...
            error(
                @printf(
                    "Passed `calculate_density_grid=true` but there is %d
    ...
    if calculate_density_grid
        ...
        molecule_species_id_for_density_grid =
            findfirst(density_grid_molecular_species .== molecular_species)
        @assert (
            (molecule_species_id_for_density_grid != 0) &&
            (molecule_species_id_for_density_grid <= length(molecule_templates))
        ) "density_grid_molecular_species not found in molecule_templates"
    end
    ...
    density_grid = Grid(
        density_grid_sim_box ? xtal.box : original_xtal_box,
        n_pts,
        zeros(n_pts...),
        :inverse_A3,
        [0.0, 0.0, 0.0]
    ...
    if verbose
        println("\tthe crystal:")
        @printf(
            "\t\treplicated (%d,%d,%d) for short-range cutoff of %f Å\n",
            repfactors[1],
            repfactors[2],
            repfactors[3],
            sqrt(ljff.r²_cutoff)
        ...
        if write_adsorbate_snapshots
            @printf(
                "\tWriting snapshots of adsorption positions every %d cycles (after burn cycles)\n",
                snapshot_frequency
            )
        ...
        if calculate_density_grid
            @printf(
                "\tTracking adsorbate spatial probability density grid of adsorbate %s, updated every %d cycles (after burn cycles)\n",
                density_grid_molecular_species,
                snapshot_frequency
            )
            @printf(
                "\t\tdensity grid voxel spacing specified as %.3f Å => %d by %d by %d voxels\n",
                density_grid_dx,
                n_pts...
            )
            if density_grid_sim_box
    ...
    for s in 1:nb_species
        if !neutral(molecule_templates[s].charges)
            error(
                @sprintf(
                    "Molecule %s is not charge neutral!\n",
                    molecule_templates[s].species
                )
            )
        end
        if !forcefield_coverage(molecule_templates[s].atoms, ljff)
            error(
                "molecule $(molecule_templates[s].species) not covered by the force field $(ljff.name)"
            )
        end
    end
    ...
    if any(molecules_species -> length(molecules_species) != 0, molecules)
        ...
        for (s, molecules_species) in enumerate(molecules)
            ...
            @assert all(
                molecule -> molecule.species == molecule_templates[s].species,
                molecules_species
            ) "initializing with wrong molecule species"
            ...
            @assert all(
                molecule ->
                    !distortion(molecule, Frac(molecule_templates[s], xtal.box), xtal.box),
                molecules_species
            ) "initializing with distorted molecules"
        end
    ...
    ####
    mc_proposal_probabilities = [0.0 for _ in 1:N_PROPOSAL_TYPES]
    # set defaults
    ...
    if verbose
        ...
        for p in 1:N_PROPOSAL_TYPES
            @printf(
                "\t\tprobability of %s: %f\n",
                PROPOSAL_ENCODINGS[p],
                mc_proposal_probabilities[p]
            )
        end
    end
    ...
    markov_chain_time = 0
    outer_cycle_start = 1
    for outer_cycle in outer_cycle_start:(n_burn_cycles + n_sample_cycles)
        if show_progress_bar
            next!(
                progress_bar;
                showvalues=[
                    (:cycle, outer_cycle),
                    (:number_of_molecules, sum(length.(molecules)))
                ]
            )
        end
        for inner_cycle in 1:(sum(length.(molecules)) + 1)
            markov_chain_time += 1
            ...
            if which_move == INSERTION
                random_insertion!(
                    molecules[which_species],
                    xtal.box,
                    molecule_templates[which_species]
                )
                ...
                if rand() <
                   fugacities[which_species] * xtal.box.Ω /
                   ((n_i + 1) * BOLTZMANN * temperature) * exp(-sum(ΔE) / temperature)
            ...
            elseif (which_move == DELETION) && (n_i != 0)
                ...
                if rand() <
                   n_i * BOLTZMANN * temperature /
                   (fugacities[which_species] * xtal.box.Ω) * exp(sum(ΔE) / temperature)
            ...
            elseif (which_move == TRANSLATION) && (n_i != 0)
                ...
                energy_old =
                    potential_energy(which_species, molecule_id, molecules, xtal, ljff)

                old_molecule = random_translation!(
                    molecules[which_species][molecule_id],
                    xtal.box,
                    adaptive_δ
                )
            ...
            elseif (which_move == ROTATION) &&
                   needs_rotations(molecule_templates[which_species]) &&
                   (n_i != 0)
            ...
            elseif (which_move == REINSERTION) && (n_i != 0)
                ...
                old_molecule =
                    random_reinsertion!(molecules[which_species][molecule_id], xtal.box)
            ...
            elseif (which_move == IDENTITY_CHANGE) && (n_i != 0)
                ...
                insert_w_random_orientation!(
                    molecules[candidate_species],
                    xtal.box,
                    molecule_templates[candidate_species],
                    old_molecule.com
                )

                # calculate the energy of the new molecule
                energy_new =
                    potential_energy(candidate_species, n_j + 1, molecules, xtal, ljff)

                # Acceptance rule for identity change
                if rand() <
                   (
                    n_i * fugacities[candidate_species] /
                    ((n_j + 1) * fugacities[which_species])
            ...
            if outer_cycle > n_burn_cycles # then we're in "production" cycles.
                if markov_chain_time % sample_frequency == 0
                    ...
                    gcmc_stats[current_block].Un +=
                        sum(system_energy) * sum(length.(molecules))
                end
            end
        end # inner cycles
        ...
        if (outer_cycle <= n_burn_cycles) &&
           (outer_cycle % floor(Int, n_burn_cycles / 12) == 0)
            adjust!(adaptive_δ)
        end
        ...
        if (outer_cycle > n_burn_cycles) && (outer_cycle % snapshot_frequency == 0)
            ...
            if calculate_density_grid
                if density_grid_sim_box
                    update_density!(
                        density_grid,
                        molecules[molecule_species_id_for_density_grid],
                        density_grid_molecular_species
                    )
                else
                    update_density!(
                        density_grid,
                        Cart.(molecules[molecule_species_id_for_density_grid], xtal.box),
                        density_grid_molecular_species
                    )
                end
            end
    ...
    # variances
    results["var(N)"] =
        (sum(gcmc_stats).n² / sum(gcmc_stats).n_samples) - (results["⟨N⟩ (molecules)"] .^ 2)

    # isosteric heat of adsorption TODO stdev of this too.
    results["Q_st (K)"] =
        temperature .-
        (
            sum(gcmc_stats).Un / sum(gcmc_stats).n_samples .-
            results["⟨U⟩ (K)"] * results["⟨N⟩ (molecules)"]
        ) ./ results["var(N)"]
    ...
    results["⟨N⟩ (molecules/unit cell)"] =
        avg_n / (repfactors[1] * repfactors[2] * repfactors[3])
    results["err ⟨N⟩ (molecules/unit cell)"] =
        err_n / (repfactors[1] * repfactors[2] * repfactors[3])
    # (molecules/unit cell) * (mol/6.02 * 10^23 molecules) * (1000 mmol/mol) *
    #    (unit cell/xtal amu) * (amu/ 1.66054 * 10^-24)
    results["⟨N⟩ (mmol/g)"] =
        results["⟨N⟩ (molecules/unit cell)"] * 1000 /
        (6.022140857e23 * molecular_weight(xtal) * 1.66054e-24) *
        (repfactors[1] * repfactors[2] * repfactors[3])
    results["err ⟨N⟩ (mmol/g)"] =
        results["err ⟨N⟩ (molecules/unit cell)"] * 1000 /
        (6.022140857e23 * molecular_weight(xtal) * 1.66054e-24) *
        (repfactors[1] * repfactors[2] * repfactors[3])

    # Markov stats
    for (proposal_id, proposal_description) in PROPOSAL_ENCODINGS
        results[@sprintf("Total # %s proposals", proposal_description)] =
            markov_counts.n_proposed[proposal_id]
        results[@sprintf("Fraction of %s proposals accepted", proposal_description)] =
            markov_counts.n_accepted[proposal_id] / markov_counts.n_proposed[proposal_id]
    end
...
function μVT_sim(
    ...
    return μVT_sim(xtal, [molecule_template], temperature, [pressure], ljff; kwargs...)
end
...
function μVT_output_filename(
    ...
    for m in 1:length(molecule_templates)
...
function print_results(results::Dict; print_title::Bool=true)
    if print_title
        ...
        @printf(
            "GCMC simulation of %s in %s at %f K and %f bar pressure, %f bar fugacity using %s forcefield.\n\n",
            results["adsorbate"],
            results["xtal"],
            results["temperature (K)"],
            results["pressure (bar)"],
            results["fugacity (bar)"] / 100000.0,
            results["forcefield"]
        )
    end
...
function pretty_print(
    ...
    println(" force field.")
    return printstyled(split(ljff.name, ".")[1]; color=:green)
end
...
"""
function stepwise_adsorption_isotherm(
    xtal::Crystal,
    molecule_templates::Array{Molecule{Cart}, 1},
    temperature::Float64,
...
function stepwise_adsorption_isotherm(
    ...
    return stepwise_adsorption_isotherm(
        xtal,
        [molecule_templates],
        temperature,
...
function adsorption_isotherm(
    ...
    function run_pressure(partial_pressures::Array{Float64, 1})
        return μVT_sim(
            xtal,
            molecule_templates,
            temperature,
            ...
            ljff;
            kwargs...
        )[1] # only return results
    end # only return results
    ...
    return results[[findall(x -> x == i, ids)[1] for i in eachindex(ids)]]
end
...
```
"""
function isotherm_sim_results_to_dataframe(
    ...
    for (i, pressure) in enumerate(pressures)
        jld2_filename = μVT_output_filename(
            xtal,
            molecule_templates,
            temperature,
            pressure,
...
function isotherm_sim_results_to_dataframe(
    ...
    return isotherm_sim_results_to_dataframe(
        desired_props,
        xtal,
        [molecule_templates],
        temperature,
