...
function electrostatic_potential_energy(framework::Framework,
    ...
    for c = 1:molecule.charges.n_charges
        ...
        @inbounds pot += molecule.charges.q[c] * ϕ(framework.charges, molecule.charges.xf[:, c], eparams, framework.box, eikar, eikbr, eikcr)
    end
    return pot
end
...
"""
@inline function ϕ(charges::Charges, xf::Array{Float64, 1}, eparams::EwaldParams, box::Box,
                   eikar::OffsetArray{Complex{Float64}},
                   eikbr::OffsetArray{Complex{Float64}},
                   eikcr::OffsetArray{Complex{Float64}})
    pot = 0.0
    ...
    for kvec in eparams.kvecs
        ...
        for c = 1:charges.n_charges
            @inbounds pot += kvec.wt * charges.q[c] * real(eikar[c, kvec.ka] * eikbr[c, kvec.kb] * eikcr[c, kvec.kc])
        end
    end
    ...
    for c = 1:charges.n_charges
        ...
        if r < eparams.sr_cutoff_r
            @inbounds @fastmath pot += charges.q[c] / r * erfc(r * eparams.α) / FOUR_π_ϵ₀
        end
    end
    return pot
end
