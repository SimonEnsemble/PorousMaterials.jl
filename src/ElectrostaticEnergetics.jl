...
* `eikra::OffsetArray{Complex{Float64}}`: array for storing e^{i * kc ⋅ r}; has indices
    -kreps[2]:kreps[1] and corresponds to recip. vectors in c-direction
* `incl_periodic_images_excluded_molecule::Bool`: include the electrostatic potential
generated by the periodic images of `molecules[exclude_molecule_id]`?
"""
...
function _intramolecular_energy(molecule::Molecule, eparams::EwaldParams)
    ...
    return ϕ_intra /= (4.0 * π * ϵ₀)
end

# interaction of molecule with its own periodic images
function _electrostatic_potential_energy_own_periodic_images(molecule::Molecule,
                                                             eparams::EwaldParams,
                                                             kvectors::Array{Kvector, 1})
    # the view here is that all other molecules create an electrostatic potential for the
    #  molecules[exclude_molecule_id] molecule.
    ...
    for charge in molecule.charges
        ...
        for kvector in kvectors
            ...
            #     e^{i kb r * vec(kc)} where r = x - xᵢ
            #   and eikar[ka], eikbr[kb], eikcr[kc] contain exactly the above components.
            @unsafe @inbounds ϕ_lr += charge.q * charge.q * kvector.wt 
        end
    end
    ϕ_lr /= eparams.box.Ω
...
function electrostatic_potential_energy(molecules::Array{Molecule, 1},
    ...
    ϕ += _electrostatic_potential_energy_own_periodic_images(molecule, eparams, kvectors)
    ϕ += _spurious_self_interaction_energy(molecules[molecule_id], eparams)
    ϕ += _intramolecular_energy(molecules[molecule_id], eparams)
    return ϕ::Float64
end
...
function total_electrostatic_potential_energy(molecules::Array{Molecule, 1},
                                              ...
                                              eikbr::OffsetArray{Complex{Float64}},
                                              eikcr::OffsetArray{Complex{Float64}})
    # molecule-molecule interactions in Ewald sum
    ϕ = 0.0
    ...
    ϕ /= 2.0 # avoid double counting
    
    # self interactions
    for molecule in molecules
        ϕ += _electrostatic_potential_energy_own_periodic_images(molecule, eparams, kvectors)
        ϕ += _spurious_self_interaction_energy(molecule, eparams)
        ϕ += _intramolecular_energy(molecule, eparams)
    end
    return ϕ::Float64
end
