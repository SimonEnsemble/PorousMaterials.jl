...
function center_of_mass(molecule::Molecule{Cart})
    ...
    if total_mass == 0.0
        ...
        for a in 1:(molecule.atoms.n)
            x_com +=
                rc[:atomic_masses][molecule.atoms.species[a]] *
                molecule.atoms.coords.x[:, a]
        end
    end
...
function Molecule(species::String; check_neutrality::Bool=true)
    ...
    if (!neutral(molecule.charges)) && check_neutrality
        error(@sprintf(
            "Molecule %s is not charge neutral! Pass
`check_neutrality=false` to ignore this error message.",
            species
        ))
    end
...
function translate_by!(molecule::Molecule{Cart}, dx::Cart)
    ...
    return translate_by!(molecule.com, dx)
end
...
function translate_to!(molecule::Molecule{Cart}, x::Cart)
    ...
    return translate_by!(molecule, dx)
end

function translate_to!(molecule::Molecule{Frac}, xf::Frac)
    dxf = Frac(xf.xf - molecule.com.xf)
    return translate_by!(molecule, dxf)
end

function translate_to!(molecule::Molecule{Frac}, x::Cart, box::Box)
    return translate_to!(molecule, Frac(x, box))
end

function translate_to!(molecule::Molecule{Cart}, xf::Frac, box::Box)
    return translate_to!(molecule, Cart(xf, box))
end
    ...
    if molecule.charges.n > 0
        ...
        if typeof(molecule.charges.coords) == Frac
            for i in 1:(molecule.charges.n)
                @printf(
                    io,
                    "\n\tcharge = %f, xf = [%.3f, %.3f, %.3f]",
                    molecule.charges.q[i],
...
function write_xyz(
    ...
    return write_xyz(atoms, filename; comment=comment) # Misc.jl
end
...
function write_xyz(
    ...
    return write_xyz(molecules, filename; comment=comment) # above
end
...
function write_xyz(
    ...
    all_molecules =
        [molecule for molecules_species in molecules for molecule in molecules_species]

    return write_xyz(all_molecules, box, filename) # above
end
...
function forcefield_coverage(molecule::Molecule, ljff::LJForceField)
    return forcefield_coverage(molecule.atoms, ljff)
end
...
function distortion(
    ...
    ref_molecule::Molecule{Frac},
    box::Box;
    atol::Float64=1e-12,
    throw_warning::Bool=true
)
    @assert molecule.species == ref_molecule.species
    if !isapprox(
        pairwise_distances(molecule.atoms.coords, box, false),
        pairwise_distances(ref_molecule.atoms.coords, box, false);
        atol=atol
    )
        return true
    end
    if !isapprox(
        pairwise_distances(molecule.charges.coords, box, false),
        pairwise_distances(ref_molecule.charges.coords, box, false);
        atol=atol
    )
        return true
    end
