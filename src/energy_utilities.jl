...
/(energy::PotentialEnergy, a::Int) = PotentialEnergy(energy.vdw / a, energy.es / a)
square(u::PotentialEnergy) = PotentialEnergy(u.vdw^2, u.es^2)
Base.sqrt(u::PotentialEnergy) = PotentialEnergy(sqrt(u.vdw), sqrt(u.es))
function std(energies::Array{PotentialEnergy, 1})
    return PotentialEnergy(
        std([en.vdw for en in energies]),
        std([en.es for en in energies])
    )
end
...
Base.sum(v::SystemPotentialEnergy) = v.gg.vdw + v.gg.es + v.gh.vdw + v.gh.es
function +(u::SystemPotentialEnergy, v::SystemPotentialEnergy)
    return SystemPotentialEnergy(u.gh + v.gh, u.gg + v.gg)
end
function -(u::SystemPotentialEnergy, v::SystemPotentialEnergy)
    return SystemPotentialEnergy(u.gh - v.gh, u.gg - v.gg)
end
...
/(u::SystemPotentialEnergy, a::Int) = SystemPotentialEnergy(u.gh / a, u.gg / a)
...
function std(us::Array{SystemPotentialEnergy, 1})
    return SystemPotentialEnergy(std([u.gh for u in us]), std([u.gg for u in us]))
end

function Base.isapprox(
    u::SystemPotentialEnergy,
    v::SystemPotentialEnergy;
    verbose::Bool=true,
    atol::Float64=1e-6
)
