...
function random_insertion!(
    ...
    box::Box,
    template::Molecule{Cart}
)
    xf_com = Frac(rand(3))
    return insert_w_random_orientation!(molecules, box, template, xf_com)
end
...
"""
function insert_w_random_orientation!(
    molecules::Array{Molecule{Frac}, 1},
    box::Box,
    template::Molecule{Cart},
    xf_com::Frac
    ...
    return push!(molecules, molecule)
end
...
function remove_molecule!(molecule_id::Int, molecules::Array{<:Molecule, 1})
    return splice!(molecules, molecule_id)
end
...
function apply_periodic_boundary_condition!(molecule::Molecule{Frac})
    ...
    return translate_to!(molecule, new_com)
end
...
function adjust!(adaptive_δ::AdaptiveTranslationStepSize)
    ...
    acceptance_rate =
        adaptive_δ.nb_translations_accepted / adaptive_δ.nb_translations_proposed
    ...
    return adaptive_δ.nb_translations_accepted = 0
end
...
function random_translation!(
    ...
    box::Box,
    adaptive_δ::AdaptiveTranslationStepSize
)
