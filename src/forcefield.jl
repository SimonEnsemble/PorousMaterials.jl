...
"""
function LJForceField(
    forcefield::String;
    r_cutoff::Float64=14.0,
    ...
    for row in eachrow(df)
        ...
        if atom_species in keys(ljff.pure_σ)
            error(
                @sprintf(
                    "Atom %s listed at least twice in %s.\n",
                    atom_species,
...
function replication_factors(unitcell::Box, ljforcefield::LJForceField)
    return replication_factors(unitcell, sqrt(ljforcefield.r²_cutoff))
end
function replication_factors(crystal::Crystal, r_cutoff::Float64)
    return replication_factors(crystal.box, r_cutoff)
end
function replication_factors(crystal::Crystal, ljforcefield::LJForceField)
    return replication_factors(crystal.box, sqrt(ljforcefield.r²_cutoff))
end
...
function forcefield_coverage(crystal::Crystal, ljff::LJForceField)
    return forcefield_coverage(crystal.atoms, ljff)
end
