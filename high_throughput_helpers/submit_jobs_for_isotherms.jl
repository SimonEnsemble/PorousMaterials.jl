...
"""
function write_gcmc_submit_script(
    ...
    @printf(
        "Writing submission script for μVT sim of %s in %s at %f bar and %f K.\n",
        gas,
        structurename,
        temperature,
        pressure
    ...
    @printf(
        ...
        joinpath(
            jobscriptdir,
            structurename * "_" * string(temperature) * "_" * string(pressure)
        ),
        joinpath(
            jobscriptdir,
            structurename * "_" * string(temperature) * "_" * string(pressure)
