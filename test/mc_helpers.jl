...
@testset "MCHelpers Tests" begin
    ...
    translate_to!(molecule, Cart([26.0, -0.2, 12.0]), box)
    ...
    for i in 1:5000
        ...
        dx_a =
            molecules[which_molecule].atoms.coords.xf -
            old_molecules[which_molecule].atoms.coords.xf
        dx_c =
            molecules[which_molecule].charges.coords.xf -
            old_molecules[which_molecule].charges.coords.xf
        dx_c = dx_c .- dx_a[:, 1]
        dx_a = dx_a .- dx_a[:, 1]
        if !(
            isapprox(dx_a, zeros(3, 3); atol=1e-8) &&
            isapprox(dx_c, zeros(3, 3); atol=1e-8)
        )
        ...
        if isapprox(old_molecule, molecules[which_molecule]) ||
           isapprox(old_molecule.com, molecules[which_molecule].com) ||
           isapprox(old_molecule.charges, molecules[which_molecule].charges) ||
           isapprox(old_molecule.atoms, molecules[which_molecule].atoms)
