...
@testset "GCMC (quick) tests" begin
    ...
    df_data = isotherm_sim_results_to_dataframe(
        desired_props,
        xtal,
        molecule,
        temperature,
    ...
    @test all(
        i -> isapprox(df_data[i, Symbol("pressure (bar)")][1], pressures[i]),
        1:length(pressures)
    )
    @test all(
        i -> isapprox(df_data[i, Symbol("⟨N⟩ (mmol/g)")][1], test_mmol_g[i]),
        1:length(test_mmol_g)
    )
