...
@testset "GCMC (long) tests" begin
    ...
    if tests_to_run["ideal_gas"]
        ...
        for i in eachindex(fugacity)
            results, molecules = μVT_sim(
                ...
                fugacity[i],
                forcefield;
                n_burn_cycles=100000,
                n_sample_cycles=100000,
                verbose=false
            )
            @printf(
                "fugacity %f, n_ig = %f, n_sim = %f\n",
                fugacity[i],
                n_ig[i],
    ...
    if tests_to_run["Xe in SBMOF-1"]
        ...
        results = stepwise_adsorption_isotherm(
            xtal,
            molecule,
            298.0,
            test_fugacities,
        ...
        for i in eachindex(test_fugacities)
            @test isapprox(
                results[i]["⟨N⟩ (molecules/unit cell)"],
                test_molec_unit_cell[i],
                rtol=0.025
            )
    ...
    if tests_to_run["CO2 in ZIF-71"]
        ...
        if run_sims
            results = stepwise_adsorption_isotherm(
                ...
                co2,
                298.0,
                convert(Array{Float64, 1}, df[:fugacity_Pa] / 100000.0),
                ff;
                n_burn_cycles=2000,
