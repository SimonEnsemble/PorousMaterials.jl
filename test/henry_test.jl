...
@testset "Henry coefficient tests" begin
    ...
    accessibility_grid, some_pockets_were_blocked = compute_accessibility_grid(
        framework,
        molecule,
        ljff;
        n_pts=(50, 50, 50),
        energy_tol=3.0 * temperature,
        verbose=true,
    ...
    result_manual_block = henry_coefficient(
        framework,
        molecule,
        temperature,
        ljff;
    ...
    @test isapprox(
        ...
        result_manual_block["henry coefficient [mmol/(g-bar)]"],
        rtol=0.01
    )
end
