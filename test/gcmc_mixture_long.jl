...
if tests_to_run["Kr/Xe in SBMOF-1"]
    ...
    raspa_data = Dict(
        :Kr => CSV.read(joinpath(pwd(), "data", "raspa_data/Kr.csv"), DataFrame),
    ...
    for (i, p) in enumerate([0.01, 0.1, 0.5])
        ...
        for (j, sp) in enumerate(adsorbates)
            ...
            @test results["pressure (bar)"][j] ≈
                  raspa_data[:mix][i, Symbol(sp * " pressure (bar)")]
            # evaluate adsorption uptake
            std_err =
                raspa_data[:mix][i, Symbol("err $sp loading (mmol/g)")] +
                results["err ⟨N⟩ (mmol/g)"][j]
            @test isapprox(
                raspa_data[:mix][i, Symbol(sp * " loading (mmol/g)")],
                results["⟨N⟩ (mmol/g)"][j],
                atol=std_err
            )
        end
        ...
        for mol in mol_templates
            if tests_to_run["run pure gas sims"]
                ...
                pure_gas_filename =
                    μVT_output_filename(xtal, [mol], temperature, [p], ljff, 50000, 50000)
                @load joinpath(rc[:paths][:simulations], pure_gas_filename) results
            end
            # make sure the pressures are correct
            @test results["pressure (bar)"][1] ≈ raspa_data[mol.species][
                i,
                Symbol(String(mol.species) * " pressure (bar)")
            ]
            # evaluate adsorption uptake
            std_err =
                raspa_data[mol.species][
                    i,
                    Symbol("err $(String(mol.species)) loading (mmol/g)")
                ] + results["err ⟨N⟩ (mmol/g)"][1]
            @test isapprox(
                raspa_data[mol.species][
                    i,
                    Symbol(String(mol.species) * " loading (mmol/g)")
                ],
                results["⟨N⟩ (mmol/g)"][1],
                atol=std_err
            )
...
if tests_to_run["ideal gas mixture"]
    ...
    n_ig =
        partial_fugacities * empty_space.box.Ω / (PorousMaterials.BOLTZMANN * temperature) *
        100000.0
    n_sim = similar(n_ig)
    for i in 1:length(partial_fugacities)
        ...
        for s in 1:2
            @printf(
                ...
                partial_fugacities[i][s],
                n_ig[i][s],
                results["⟨N⟩ (molecules/unit cell)"][s]
            )
        end
        # test results
